...
import { generateIdentityKeyPair, storeIdentityKey } from '../shared/crypto/e2ee';
...
export function RegisterScreen({ navigation }: Props) {
  ...
  async function handleRegister() {
    if (!username.trim() || password.length < 8) {
      Alert.alert(
        'Проверьте данные',
        'Имя пользователя обязательно, пароль — минимум 8 символов',
      );
      return;
    }
    ...
    >
      ...
      >
        <Animated.View entering={FadeInUp.duration(600).springify()} style={styles.logo}>
          ...
          <Text style={styles.tagline}>Создайте аккаунт</Text>
        </Animated.View>
        ...
        <Animated.View entering={FadeInDown.delay(100).duration(500).springify()} style={styles.card}>
          ...
          <View style={styles.hint}>
            <Text style={styles.hintText}>
              При регистрации автоматически создаётся пара криптографических ключей,
              которые хранятся только на этом устройстве.
            </Text>
          </View>
